# 🛍️ ROOMナビ — 楽天ROOMアフィリ支援アプリ

楽天ROOMで稼ぐための「投稿ボタンを押す直前まで」を自動化する単一HTMLアプリ。
リサーチ（何を載せるか）→ 投稿文生成 → ストック管理 → 実績記録 を1つにまとめています。

> **なぜ全自動投稿ではないのか**：楽天ROOMには公開の投稿APIがなく、
> 自動投稿ボットは利用規約違反（発覚するとアカウント停止＝収益ゼロ）です。
> そのため本アプリは規約の範囲内で、投稿以外の全工程を自動化します。

## 公開版（ふだんはこちらを使う）

**https://kaneshiro0101-pixel.github.io/room-navi/** （GitHub Pages。スマホは「ホーム画面に追加」推奨）

> 2026年の楽天API新仕様により、商品検索は楽天側に登録したドメイン（上記URL）からしか動きません。
> `index.html` をダブルクリックで開いても検索以外（投稿文生成・ストック・実績）は全部使えます。

## 使い方（最初の1回だけ）

1. 上記URLを開く
2. 「⚙️設定」タブで楽天APIキーを保存
   - **アプリID（必須）**: [楽天ウェブサービス](https://webservice.rakuten.co.jp/) でアプリ作成すると表示（無料）
   - **アクセスキー（必須）**: アプリIDと同じ画面に表示される。2026年新APIから必須
   - **アフィリエイトID**: [楽天アフィリエイト](https://affiliate.rakuten.co.jp/) の管理画面に表示（例 `1a2b3c4d.5e6f7a8b`）
   - ※アフィリエイトIDを入れないと報酬が発生するリンクになりません（アプリが警告します）
   - ※楽天側の「許可されたWebサイト」に `kaneshiro0101-pixel.github.io` の登録が必要

## 毎日の流れ（1回10分）

1. **🔍リサーチ**: キーワード検索 or ジャンル別ランキングで商品を取得。
   「スコア順」= 1件あたり報酬(円) × 売れ筋度(レビュー数・評価) × 売れやすい価格帯(1,000〜9,999円)ボーナス。
   上位から「✍️投稿文を作る」を押すだけ。
2. **✍️投稿文**: テンプレ6種（悩み解決/愛用レビュー風/比較/季節/お得セール/呼びかけ）＋ハッシュタグ＋レビュー実績(★と件数)で自動生成。
   **投稿文スコア**が常時表示され、ROOMの定石（冒頭28字フック・本文80〜150字・ベネフィット明記・タグ3〜8個・NGワードなし）に沿って採点＆改善アドバイス。
   もっと自然な文が欲しい時は「🤖AI用プロンプトをコピー」→ Claude に貼ると3案出ます。
3. **投稿**: 「📋コピー」→「🔗ROOMを開く」→ ROOMで商品を検索して「コレ!」→ コメント欄にペースト。
   目安は1日3〜5投稿。
4. **📣SNS展開**: 同じ商品を X / Threads / Pinterest 向けにも自動生成（全て#PR明示・楽天アフィリリンク付き・文字数最適化）。
   「3つまとめてSNSストックへ」→ ストックタブの「SNS書き出し」で `sns-room.json` を出力 → 自動投稿ツール（sns-auto-poster）へ。
5. **📦ストック**: 書き溜めて下書き→投稿済みを管理。今日の投稿数も表示。
5. **📈実績**: 楽天アフィリエイトのレポートからクリック/成果/報酬を転記→月次集計とグラフ。

## データについて

- 保存は端末内の localStorage のみ（キー `roomNavi_v1`）。外部送信なし（楽天APIへの検索を除く）
- バックアップ: 設定/ストックタブから JSON 書き出し・取り込み（**冪等**＝同じファイルを何度取り込んでも重複しない）
- オフラインでも投稿文生成・ストック・実績は動作（商品検索のみネット必要）

## 自動テスト

```
cd C:\Users\user\rakuten-room-navi
npm test
```

Node標準の `node --test` で 68テスト。`index.html` 内の `/*CORE_START*/〜/*CORE_END*/` の
純ロジック（`RoomCore`）を抽出して検証します（URL構築 / API応答の正規化 / 報酬スコア計算 /
投稿文生成 / 冪等マージ / 実績集計 など）。**index.html のコアを直したら必ず `npm test` を通すこと。**

## ファイル構成

```
rakuten-room-navi/
├── index.html      … アプリ本体（1ファイル完結・これだけで動く）
├── README.md       … このファイル
├── package.json    … npm test 用
├── docs/plan.md    … 設計メモ
└── test/core.test.js … 自動テスト（37件）
```

## 更新の反映方法（開発メモ）

`index.html` を改修したら: `npm test` → `deploy/index.html` へコピー → `git commit` → `git push`。
GitHub Pagesが自動再ビルドする（詰まったら `gh api repos/kaneshiro0101-pixel/room-navi/pages/builds -X POST`）。

## 制限・注意

- 楽天APIは約1リクエスト/秒の制限あり。連打すると429エラーになる（少し待てばOK）
- 楽天APIは2026年に新仕様へ移行済み（`openapi.rakuten.co.jp`・accessKey必須・登録ドメイン制限）。本アプリは新仕様対応
- 投稿文の誇大表現（「絶対痩せる」等）は薬機法・景表法NG。テンプレとAIプロンプトはNG表現を避ける設計
- Netlify等に公開する場合もAPIキーは各端末のlocalStorageに保存される（HTMLには埋め込まれない）
