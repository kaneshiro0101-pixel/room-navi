...
// ---- テンプレの自動移行 ----
test('migrateTemplates: 旧デフォルト4種のままなら新6種に置き換える', () => {
  const old4 = [
    { id: 'nayami', name: '悩み解決', body: '旧本文1' },
    { id: 'review', name: '愛用レビュー風', body: '旧本文2' },
    { id: 'hikaku', name: '比較・選び方', body: '旧本文3' },
    { id: 'season', name: '季節・イベント', body: '旧本文4' }
  ];
  const r = Core.migrateTemplates(old4);
  assert.equal(r.changed, true);
  assert.ok(r.templates.length >= 6);
});

test('migrateTemplates: 自作テンプレが混ざっていればそのまま維持', () => {
  const custom = [
    { id: 'nayami', name: '悩み解決', body: '旧本文1' },
    { id: 'tpl99', name: '俺の自作', body: '自作本文' }
  ];
  const r = Core.migrateTemplates(custom);
  assert.equal(r.changed, false);
  assert.equal(r.templates.length, 2);
  assert.equal(r.templates[1].name, '俺の自作');
});

test('migrateTemplates: 空・不正なら新デフォルトを返す', () => {
  assert.ok(Core.migrateTemplates(null).templates.length >= 6);
  assert.ok(Core.migrateTemplates([]).templates.length >= 6);
});

test('migrateTemplates: 新6種はそのまま(changed=false)', () => {
  const r = Core.migrateTemplates(JSON.parse(JSON.stringify(Core.DEFAULT_TEMPLATES)));
  assert.equal(r.changed, false);
  assert.equal(r.templates.length, Core.DEFAULT_TEMPLATES.length);
});
